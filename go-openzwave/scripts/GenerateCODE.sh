...
enumerate()
{
    x=0; cat /libs/openzwave-1.4.193/cpp/src/Notification.h | grep Code | grep Code_ | sed "s/^.*Code/Code/" | sed "s/Code_//" | sed "s/[^A-Za-z].*//" | while read n; do echo $x $n; let x=x+1; done
}
